...
if [ ! -f .netlify/state.json ]; then
    ...
    echo "   Site name: seochecksite (or press Enter for auto-generated)"
...
SITE_URL=$(netlify status --json 2>/dev/null | grep -o '"url":"[^"]*' | head -1 | cut -d'"' -f4 || echo "")
if [ -n "$SITE_URL" ]; then
    netlify env:set NEXT_PUBLIC_SITE_URL "$SITE_URL"
    echo "✅ NEXT_PUBLIC_SITE_URL set to: $SITE_URL"
else
    echo "⚠️  Could not auto-detect site URL. Please set NEXT_PUBLIC_SITE_URL manually in Netlify dashboard"
fi
...
echo "   2. Netlify will auto-deploy, or run: netlify deploy --prod"
